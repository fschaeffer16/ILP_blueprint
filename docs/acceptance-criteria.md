...
| AC-02 | Every generated lesson and item displays its objective, version and source trace. | 🟡 Objective/version on every manifest (`lockedContract`); source-trace display pending lesson player |
...
| AC-04 | Teacher can inspect and override every consequential ILP inference. | 🟡 Model supports it (`teacherConfirmed`, force/disable adaptations); teacher UI pending |
| AC-05 | Bot behavior changes correctly between lesson, homework, quiz and exam modes. | ⬜ Bot slice pending (`botMode` modeled) |
| AC-06 | AI grading displays criterion-level evidence and cannot release a final grade. | ⬜ Grading slice pending (spec §36) |
| AC-07 | A failed objective produces a different remediation lesson and equivalent reassessment. | ⬜ Remediation slice pending (spec §37) |
| AC-08 | A classwide 75% failure suspends the affected grade and generates an audit card. | ⬜ 75% workflow pending (spec §37) |
| AC-09 | Teacher dashboard prioritizes actionable items and suppresses duplicate noise. | ⬜ Teacher command center pending |
...
| AC-12 | All AI recommendations, teacher changes, content versions and grade releases are auditable. | 🟡 Manifests carry rationale + integrity results; full audit log pending |
